// Number to Words converter for Indian Rupee Currency (English & Marathi)
...
function numberToWordsEnglish(num) {
  ...
  if (remainder > 0) {
    words += convertLessThanThousandEn(remainder);
  }

  return `${words.trim()} Rupees Only`;
}

// Marathi Number Words mapping
const marathiUnits = {
...
function numberToWordsMarathi(num) {
  ...
  if (crore > 0) {
    words.push(`${marathiUnits[crore] || crore} कोटी`);
  }
  if (lakh > 0) {
    words.push(`${marathiUnits[lakh] || lakh} लाख`);
  }
  if (thousand > 0) {
    words.push(`${marathiUnits[thousand] || thousand} हजार`);
  }
  if (hundred > 0) {
